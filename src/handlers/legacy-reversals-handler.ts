...
export async function legacyReversalHandler ({ mojaClient, logger }: AdaptorServices, financialRequest: LegacyReversalRequest): Promise<void> {
  try {
    const transaction = await Transaction.query().where('transactions.lpsKey', financialRequest.lpsKey).withGraphJoined('[lpsMessages, payer, payee]').where('lpsMessages.id', financialRequest.lpsFinancialRequestMessageId).first().throwIfNotFound()
